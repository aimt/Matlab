...
while nFrame>0
    ...
    if ~isempty(bbox)  %  Get the centroid of remaining blobs
        ...
        predict(2,i)=uint16(centY);
         ...
         movL =  predict(1,mod(i+1,14)+1);
         movU =  predict(2,mod(i+1,14)+1);

          movL1 =  predict(1,mod(i+3,14)+1);
         movU1 =  predict(2,mod(i+3,14)+1);

          movL2 =  predict(1,mod(i+5,14)+1);
         movU2 =  predict(2,mod(i+5,14)+1);

          movL3 =  predict(1,mod(i+7,14)+1);
         movU3 =  predict(2,mod(i+7,14)+1);

          movL4 =  predict(1,mod(i+11,14)+1);
         movU4 =  predict(2,mod(i+11,14)+1);
  ...
  nFrame = nFrame -1;
         ...
         elseif ( movL < movL1 &&  movL1 < movL2 &&  movL2 < movL3 &&  movL3 < movL4 )
           disp(predict(1,i)-movL4);
           if (predict(1,i)-movL4 > 10)
                 disp('Right');
